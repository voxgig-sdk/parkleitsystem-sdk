...
boru-driven command-line client **and** interactive REPL for the Parkleitsystem
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parkleitsystem-cli)
make build

# 2. See usage (words, entities, env vars)
./parkleitsystem-cli --help

# 3. Provide credentials once, via the environment
export PARKLEITSYSTEM_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./parkleitsystem-cli list get_all_city
./parkleitsystem-cli list get_city_parking_info

# 5. Override the API base URL for a single call
PARKLEITSYSTEM_BASE=https://api.example.com ./parkleitsystem-cli list get_all_city

# 6. No arguments -> interactive REPL
./parkleitsystem-cli
parkleitsystem> list get_all_city
parkleitsystem> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parkleitsystem-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/parkleitsystem-cli list get_all_city
   ```
...
Run with no arguments to open a REPL (prompt `parkleitsystem>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_all_city`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/parkleitsystem-cli`. |
...
(`./parkleitsystem-cli <expr>`) and interactively (the REPL), and expressions compose the
