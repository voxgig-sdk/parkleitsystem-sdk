...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_all_city | get_city_parking_info"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ParkleitsystemSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "parkleitsystem_list",
		Description: "List records from Parkleitsystem. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "parkleitsystem_load",
		Description: "Load a single record from Parkleitsystem. " +
...
func runOp(client *sdk.ParkleitsystemSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ParkleitsystemSDK, name string) (sdk.ParkleitsystemEntity, error) {
	switch strings.ToLower(name) {
	case "get_all_city":
		return client.GetAllCity(nil), nil
	case "get_city_parking_info":
		return client.GetCityParkingInfo(nil), nil
