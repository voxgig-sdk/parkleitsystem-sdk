...
[MCP](https://modelcontextprotocol.io) server exposing the Parkleitsystem SDK as
two agent tools — `parkleitsystem_list` and `parkleitsystem_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/parkleitsystem-mcp)
make build

# 2. Provide credentials via the environment
export PARKLEITSYSTEM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user parkleitsystem \
  -- /absolute/path/to/parkleitsystem-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./parkleitsystem-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// parkleitsystem_list: first page of records
{ "entity": "get_all_city" }
{ "entity": "get_all_city", "query": { } }

// parkleitsystem_load: one record by id
{ "entity": "get_all_city", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/parkleitsystem-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user parkleitsystem \
     -- "$PWD"/dist/*/parkleitsystem-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `parkleitsystem_list` and `parkleitsystem_load` tools now appear
   in new sessions. Ask the agent to *"list get_all_city using parkleitsystem"*
   and it calls `parkleitsystem_list` with `{"entity":"get_all_city"}`.
...
```sh
export PARKLEITSYSTEM_APIKEY=sk_live_xxx            # API key
export PARKLEITSYSTEM_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `parkleitsystem_list` tool
...
### Call the `parkleitsystem_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `parkleitsystem_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `parkleitsystem_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PARKLEITSYSTEM_APIKEY` | API key sent with every request. |
| `PARKLEITSYSTEM_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

get_all_city | get_city_parking_info

### Smoke test via HTTP (raw JSON-RPC)

```sh
./parkleitsystem-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"parkleitsystem_load","arguments":{"entity":"get_all_city","query":{"id":1}}}}'
```
