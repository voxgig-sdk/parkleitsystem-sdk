...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ParkleitsystemOmni.make_runner(TEST_JSON_FILE, ParkleitsystemSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    ParkleitsystemError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ParkleitsystemOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = ParkleitsystemOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def auth_credential(client)
    utility = client.get_utility
    ctx = make_test_ctx(client, utility, nil)
    ctx.spec = ParkleitsystemSpec.new({ "headers" => {}, "query" => {}, "step" => "s" })
