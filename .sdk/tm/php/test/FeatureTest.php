...
// Parkleitsystem SDK feature test
//
...
class FtCtrl extends ParkleitsystemControl
{
...
class FtClient
{
    ...
    private ParkleitsystemUtility $utility;

    public function __construct(ParkleitsystemUtility $utility, array $options)
    {
    ...
    public function get_utility(): ParkleitsystemUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ParkleitsystemContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ParkleitsystemContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ParkleitsystemContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ParkleitsystemConfig::make_feature($name);
    ...
    private static function buildUrl(ParkleitsystemSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ParkleitsystemError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ParkleitsystemResponse($fetched);
        }
    ...
    private function fail(ParkleitsystemContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ParkleitsystemContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
