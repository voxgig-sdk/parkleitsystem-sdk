...
// Parkleitsystem SDK retry feature
...
class ParkleitsystemRetryFeature extends ParkleitsystemBaseFeature
{
    ...
    public function init(ParkleitsystemContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ParkleitsystemContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ParkleitsystemContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ParkleitsystemContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
