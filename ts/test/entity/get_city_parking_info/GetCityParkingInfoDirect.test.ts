...
describe('GetCityParkingInfoDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ParkleitsystemSDK({
  ...
  test('direct-list-get_city_parking_info', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_city_parking_info', setup.live)) return
    if (skipIfMissingIds(t, setup, ["get_city_parking_info01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PARKLEITSYSTEM_TEST_GET_CITY_PARKING_INFO_ENTID']
