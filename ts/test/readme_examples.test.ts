...
//     (\`new ParkleitsystemSDK(...)\` / \`ParkleitsystemSDK.test(...)\` become
//     \`ParkleitsystemSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ParkleitsystemSDK'
...
const TEST_SEED = {"entity":{"get_all_city":{"test01":{"id":"test01"}},"get_city_parking_info":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ParkleitsystemSDK(...)\` and any
// \`ParkleitsystemSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ParkleitsystemSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ParkleitsystemSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
