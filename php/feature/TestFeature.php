...
// Parkleitsystem SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ParkleitsystemTestFeature extends ParkleitsystemBaseFeature
{
    ...
    public function init(ParkleitsystemContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ParkleitsystemContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ParkleitsystemParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ParkleitsystemContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ParkleitsystemContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ParkleitsystemParam::call($ctx, $k);
