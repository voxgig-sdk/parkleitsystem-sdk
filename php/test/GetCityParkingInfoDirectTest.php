...
class GetCityParkingInfoDirectTest extends TestCase
{
    public function test_direct_list_get_city_parking_info(): void
    {
        $setup = get_city_parking_info_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-get_city_parking_info", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["get_city_parking_info01"] as $_liveKey) {
