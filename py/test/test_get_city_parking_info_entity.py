...
class TestGetCityParkingInfoEntity:

    def test_should_create_instance(self):
        testsdk = ParkleitsystemSDK.test(None, None)
        ent = testsdk.GetCityParkingInfo(None)
    ...
    def test_should_stream(self):
        ...
        base = ParkleitsystemSDK.test(seed, None)
        seen = list(base.GetCityParkingInfo(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = ParkleitsystemSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.GetCityParkingInfo(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _get_city_parking_info_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "get_city_parking_info." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PARKLEITSYSTEM_TEST_GET_CITY_PARKING_INFO_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_city_parking_info_ref01_data = None
        if len(get_city_parking_info_ref01_data_raw) > 0:
            get_city_parking_info_ref01_data = helpers.to_map(get_city_parking_info_ref01_data_raw[0][1])

        # LIST
        get_city_parking_info_ref01_ent = client.GetCityParkingInfo(None)
...
def _get_city_parking_info_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/get_city_parking_info/GetCityParkingInfoTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("PARKLEITSYSTEM_TEST_GET_CITY_PARKING_INFO_ENTID"))
