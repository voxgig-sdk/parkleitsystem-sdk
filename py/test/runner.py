# Parkleitsystem SDK test runner

from __future__ import annotations
import os
import json

from parkleitsystem_sdk.utility.voxgig_struct import voxgig_struct as vs


class ParkleitsystemTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ParkleitsystemTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ParkleitsystemTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ParkleitsystemTestRunner.getenv("PARKLEITSYSTEM_TEST_LIVE")
        override = ParkleitsystemTestRunner.getenv("PARKLEITSYSTEM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ParkleitsystemTestRunner.getenv(key)
        ...
        explain = ParkleitsystemTestRunner.getenv("PARKLEITSYSTEM_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PARKLEITSYSTEM_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ParkleitsystemTestRunner._test_control is not None:
            return ParkleitsystemTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ParkleitsystemTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ParkleitsystemTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ParkleitsystemTestRunner.load_test_control()
...
def load_env_local():
    ParkleitsystemTestRunner.load_env_local()


def env_override(m):
    return ParkleitsystemTestRunner.env_override(m)


def entity_data(v):
    return ParkleitsystemTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ParkleitsystemTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ParkleitsystemTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ParkleitsystemTestRunner.load_test_control()


def live_client_options():
    return ParkleitsystemTestRunner.live_client_options()


def live_delay_ms():
    return ParkleitsystemTestRunner.live_delay_ms()
