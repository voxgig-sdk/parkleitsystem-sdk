...
The Python SDK for the Parkleitsystem API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetAllCity(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Parkleitsystem(None).load(
    {"id": "test01"}, None
)
