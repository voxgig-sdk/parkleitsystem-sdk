...
from parkleitsystem_sdk.parkleitsystem_types import (
    GetAllCity,
    GetAllCityListMatch,
)
...
class GetAllCityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetAllCityEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[GetAllCity]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.GetAllCity().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
