...
import (
	...
	omni "github.com/voxgig-sdk/parkleitsystem-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ParkleitsystemError); is {
				out := map[string]any{
					"name":    "ParkleitsystemError",
...
func omniCtx(arg any, client *sdk.ParkleitsystemSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ParkleitsystemSDK); is {
			client = live
