...
module ParkleitsystemUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ParkleitsystemResponse.new({})
    ...
    if fetch_err
      ...
      response = ParkleitsystemResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ParkleitsystemResponse.new(fetched)
    else
