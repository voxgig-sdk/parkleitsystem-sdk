...
class GetCityParkingInfoEntity
  ...
  def make
    opts = @_entopts.dup
    GetCityParkingInfoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ParkleitsystemHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetCityParkingInfo, Hash] the current GetCityParkingInfo data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ParkleitsystemHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetCityParkingInfo fields)
  def match_get
  ...
  #
  # @param reqmatch [GetCityParkingInfoListMatch, Hash, nil] match filter (any subset of
  #   GetCityParkingInfo fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetCityParkingInfo>, Array] the matching GetCityParkingInfo items; raises ParkleitsystemError on failure
