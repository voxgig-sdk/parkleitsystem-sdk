...
require_relative 'Parkleitsystem_types'
...
class ParkleitsystemSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ParkleitsystemHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ParkleitsystemHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ParkleitsystemFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ParkleitsystemUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ParkleitsystemHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ParkleitsystemHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ParkleitsystemHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ParkleitsystemError => err
    ...
    if fetched.is_a?(Hash)
      status = ParkleitsystemHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ParkleitsystemError.new(
        "graphql_error", "ParkleitsystemSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ParkleitsystemSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
