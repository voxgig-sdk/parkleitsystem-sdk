...
describe("GetCityParkingInfoDirect", function()
  it("should direct-list-get_city_parking_info", function()
    local setup = get_city_parking_info_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-get_city_parking_info", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"get_city_parking_info01"}) do
...
function get_city_parking_info_direct_setup(mockres)
