...
The Lua SDK for the Parkleitsystem API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetAllCity(nil):list(nil, nil)
...
local result, err = client:Parkleitsystem(nil):load(
  { id = "test01" }, nil
)
